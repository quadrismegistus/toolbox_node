...
async function with_model(opts,log=console.log,progress=console.log) {
	...
	Model.build_vecdb = function(opts) {
		db=Model.get_db()
		this.orig_vocab.forEach(function(word,i) {
			console.log(i,word)
			db_key=word
			db_val=this.M.getVector(word).values
			// console.log('db_key',db_key)
			// console.log('db_val',db_val)
			db.set(db_key,db_val)
			// console.log('')
		})
	...
	Model.get_vector = async function(opts) {
	...
	// 	formula_str=word_or_formula
	// 	formula_str_q=formula_str.trim().split('[').join('').split(']').join('')
	// 	Model.log('locating vector position for: '+formula_str_q)

	// 	cached_formula_vec=await db.get(formula_str_q)
	// 	console.log('cached_formula_vec:::',opts['word'],formula_str_q,cached_formula_vec)

	// 	if(cached_formula_vec!=undefined) { 
	// 		vector_promise = new Promise(function(res,rej) { res(cached_formula_vec) })
	// 	} else {
	// 		// otherwise...
	// 		// get the words involved in the formula
	// 		vector_promise = new Promise(function(res,rej) {
	// 			var words_involved = split_words_only(word_or_formula)
	// 			var word2vecs = {}
	// 			var uncached_vecs=[]
	// 			console.log('words_involved',words_involved)

	// 			// loop over those composite words
	// 			words_involved.forEach(async function(w) { 
	// 				if(w!=undefined) {
	// 					cached_word_vec = await db.get(w)
	// 					console.log('cached_word_vec!',w,cached_word_vec)
	// 					if(cached_word_vec!=undefined) {
	...
	// 						w_periodized = periodize([w],opts['periods'])
	// 						console.log('w_periodized',w_periodized)
	// 						await w_periodized.forEach(async function(word_period) {
	// 							console.log(word_period,'!?')
	// 							// try cache one last time
	// 							cached_word_period_vec = await db.get(word_period)
	// 							console.log('cached_word_period_vec',word_period,cached_word_period_vec,'!?')
	...
	// 	}
	// 	return await vector_promise
	// }


	// Model.get_vector1 = async function(opts) {
		
	// 	vec_promise = new Promise(function(resolve,reject) {
	// 		db=Model.get_db()
	// 		word_or_formula = opts['word']
	// 		formula_str=word_or_formula
	// 		formula_str_q=formula_str.trim().split('[').join('').split(']').join('')
	...
	// 		db.get(formula_str_q).then(async function(cached_formula_vec) {
	// 			// if we have a cache, use it
	// 			if(cached_formula_vec!=undefined) { return resolve(cached_formula_vec) }

	// 			// otherwise...
	// 			// get the words involved in the formula
	// 			formula_promise=new Promise(function(res,rej) {
	// 				var words_involved = split_words_only(word_or_formula)
	// 				var word2vecs = {}
	// 				var uncached_vecs=[]
	// 				console.log('words_involved',words_involved)
	...
	// 					subword_promise = new Promise(function(res3,rej2) {
	// 						db.get(w).then(function(cached_word_vec) {
	// 							console.log('db_res!',cached_word_vec,w)
	// 							//if(db_res!=undefined) { return db_res }

	// 							// if we have a cache for this, set it to the word2vec formula dict
	// 							if(cached_word_vec!=undefined) {
	// 								return res3(cached_word_vec)
	// 							} else {
	// 								// still no cache?
	// 								// maybe it has no period and we need to periodize
	// 								// try an average?
	// 								word_vecs_to_avg = []
	// 								w_periodized = periodize([w])
	// 								console.log('w_periodized',w_periodized)
	// 								w_periodized.forEach(function(word_period) {
	// 									console.log(word_period,'!?')
	// 									// try cache one last time
	// 									db.get(word_period).then(function(cached_word_period_vec) {
	// 										console.log('cached',word_period,cached_word_period_vec,'!?')
	// 										if(cached_word_period_vec!=undefined) {
	// 											word_vecs_to_avg.push(cached_word_period_vec)
	// 										}
	// 									})
	// 								})
	// 								word_vec_avg = math.add(...word_vecs_to_avg)
	// 								return res3(word_vec_avg)
	// 								// word2vecs[w]=word_vec_avg
	// 							}
	// 						});
	// 					}) // end of subword promise

	// 					w_vec = await subword_promise
	// 					word2vecs[w]=w_vec
	// 				})
	// 				console.log('word2vecs in promise',word2vecs)
	// 				res(word2vecs)
				
	// 			})

	// 			word2vecs = await formula_promise
	...
	// 			console.log('vec_res!',formula_str_q,vec_res)
	// 			// vec_res = await formula_promise
	// 			return resolve(vec_res)
	// 		})
	// 	})

	// 	vec_res = await vec_promise
	// 	return vec_res
	// }

	// Model.get_vector0 = function(opts) {
	// 	word_or_formula = opts['word']
